...
const Navbar = () => {
  ...
  const API_KEY = process.env.REACT_APP_API_KEY_TMDB; // Replace with your TMDB API key
      ...
      <div className="w-[98%] max-w-7xl mx-auto py-4 flex items-center justify-between gap-4">
        ...
        <div className="flex-1 flex justify-center items-center relative">
          <div className="w-full max-w-[400px]">
            <input
              type="text"
              className="w-full py-2 px-4 bg-transparent text-white rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
              placeholder="Search for movies..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            {searchTerm && searchResults.length > 0 && (
              <ul className="absolute w-full mt-2 bg-gray-800 rounded-lg max-h-60 overflow-y-auto shadow-lg z-50">
        ...
        <div className="flex-1 flex justify-end items-center">
          ...
          <div
            ...
            } absolute top-full left-0 w-full bg-gray-900 bg-opacity-90 md:bg-transparent md:static md:flex gap-x-5 text-lg text-white`}
          >
            <Link
              to="/popular"
